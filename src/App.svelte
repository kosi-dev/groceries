...
<svelte:head>
	...
	<link rel='stylesheet' href='./assets/groceries.css'>
</svelte:head>

<br>
<div style='display: flex; width: 100%;'>		
	<div style='width:32%; padding-right: 100px; align-self: flex-end;'>
		<ul>
			<li><input id='trumf' checked type='checkbox' on:click={onclick_trumf} class='checkbox'> KIWI PLUSS</li>	
			<li><input id='store' type='checkbox' on:click={onclick_store} class='checkbox'> Butikk</li>
			<li><input id='brand' type='checkbox' on:click={onclick_brand} class='checkbox'> Produsent</li>
			<li><input id='price' checked type='checkbox' on:click={onclick_price} class='checkbox'> Pris</li>
			<li><input id='weight' type='checkbox' on:click={onclick_weight} class='checkbox'> Vekt</li>
			<li><input id='discount' type='checkbox' on:click={onclick_discount} class='checkbox'> Rabatt</li>
			<li><input id='price_kg' checked type='checkbox' on:click={onclick_price_kg} class='checkbox'> Pris / kg</li>
			<li><input id='calories' checked type='checkbox' on:click={onclick_calories} class='checkbox'> Pris / 2000 kcal</li>
		</ul>
		
		<input id='search' type='text' placeholder='Søk..' on:input={grocery_search} autocomplete='off' class='search'>	
	</div>
	<div style='width:64%;'>
		<h2>Velkommen!</h2>
		...
		<!-- <p>Du kan også se på <a href='groceries_recipe.html'>ferdige menyer</a>.</p> -->
...
<script>
	function grocery_search() {
		grocery_table.search()
	}
	function onclick_trumf() {
		grocery_table.edit_kiwi_pluss();
		grocery_list.calculate()
	}
	function onclick_store() {
		grocery_table.edit_columns(COLUMN.STORE)
	}
	function onclick_brand() {
		grocery_table.edit_columns(COLUMN.BRAND)
	}
	function onclick_price() {
		grocery_table.edit_columns(COLUMN.PRICE)
	}
	function onclick_weight() {
		grocery_table.edit_columns(COLUMN.WEIGHT)
	}
	function onclick_discount() {
		grocery_table.edit_columns(COLUMN.DISCOUNT)
	}
	function onclick_price_kg() {
		grocery_table.edit_columns(COLUMN.PRICE_KG)
	}
	function onclick_calories() {
		grocery_table.edit_columns(COLUMN.CALORIES)
	}
